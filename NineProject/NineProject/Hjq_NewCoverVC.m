...
- (void)viewDidLoad {
    ...
    //如果是前置拍的照，要进行反转处理
    if (self.picer.sourceType==UIImagePickerControllerSourceTypeCamera) {
...
{
    self.saveImage = [self addImage:self.layerImage toImage:self.imageView.image];//合并图片
...
- (void)image:(UIImage*)image didFinishSavingWithError:(NSError*)error contextInfo:(void*)contextInfo
{
    ...
    [self.picer dismissViewControllerAnimated:YES completion:^{
    }];
    [self.navigationController popViewControllerAnimated:YES];
}
@end
